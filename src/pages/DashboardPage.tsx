...
import {
  IconDumbbell, IconFlame, IconTrophy, IconChart, IconChevron,
    ...
    <div className="page">
      <div className="spread" style={{ marginBottom: 22 }}>
        <div className="row" style={{ gap: 12 }}>
          ...
          <div>
            <div className="eyebrow">{greet}{adminElevated ? " · Admin" : ""}</div>
      ...
      <div className="hero-card" onClick={() => next && nav(`/app/workouts/${next.id}`)} style={{ cursor: next ? "pointer" : "default", marginBottom: 16 }}>
        <div className="eyebrow">Proximo treino</div>
        {next ? (
          <div className="spread" style={{ marginTop: 8 }}>
            <div>
              <h2 style={{ fontSize: 24 }}>{next.name}</h2>
              <p className="text-2 numeric" style={{ fontSize: 14, marginTop: 4 }}>
                {next.exerciseCount} exercicios · {next.lastCompletedAt ? `ultimo ${new Date(next.lastCompletedAt).toLocaleDateString("pt-BR")}` : "nunca feito"}
              </p>
            </div>
      ...
      <div className="stat-grid">
        <div className="stat-card">
          ...
          <div className="stat-card__label">Treinos no mes</div>
        </div>
        <div className="stat-card">
        ...
        <div className="stat-card">
          ...
          <div className="stat-card__label">{progress ? progress.name : "Maior evolucao"}</div>
        </div>
        <div className="stat-card">
        ...
        {workouts?.slice(0, 3).map((w) => (
          <div key={w.id} className="workout-item" onClick={() => nav(`/app/workouts/${w.id}`)} style={{ cursor: "pointer" }}>
            <div className="workout-item__body">
              ...
              <div className="workout-item__meta numeric">{w.exerciseCount} exercicios · {w.sessionCount} execucoes</div>
            </div>
